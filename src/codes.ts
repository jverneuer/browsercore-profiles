 ...
 * {@link ProfileError} at projection time, so the tables double as an
 * allow-list that keeps profile data honest.
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │ DUPLICATED IN @browsercore/tls/src/iana/                                 │
 * │                                                                         │
 * │ The same IANA tables exist in @browsercore/tls (cipher-suites.ts,       │
 * │ named-groups.ts, signature-schemes.ts, versions.ts). This is            │
 * │ intentional — no dependency between the two packages. When adding a     │
 * │ new cipher suite, named group, or signature scheme, update BOTH copies. │
 * │ The IANA TLS registries barely change (once every few years), so the    │
 * │ maintenance cost of two copies is lower than a shared package.          │
 * └─────────────────────────────────────────────────────────────────────────┘
 *
 * Registries:
